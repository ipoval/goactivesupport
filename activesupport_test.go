...
import (
	...
	gas "github.com/ipoval/goactivesupport"
...
func TestNumberToStrFast(t *testing.T) {
	...
	for _, c := range testCases {
		result := gas.NumberToStrFast(c.given)
		if result != c.want {
...
func TestMethodName(t *testing.T) {
	f := func() {}
	assert.Equal(t, "github.com/ipoval/goactivesupport_test.TestMethodName.func1", gas.MethodName(f))
}

func TestStrValidLuhnCreditCardNumber(t *testing.T) {
	testCases := []struct {
		description string
		input       string
		ok          bool
	...
	for _, test := range testCases {
		if ok := gas.StrValidLuhnCreditCardNumber(test.input); ok != test.ok {
...
func BenchmarkStringValidLuhnCreditCardNumber(b *testing.B) {
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		gas.StrValidLuhnCreditCardNumber("2323 2005 7766 3554")
	}
}

func TestBcryptHashStr(t *testing.T) {
	word := "test"
	hash, err := gas.BcyptHashStr(word)
